...
import { Upload, Filter, UserPlus, Trash2, Plus, Check, Search, ArrowUpDown, X, Pencil, ChevronDown, ChevronRight, FileWarning } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    ...
    // Special calculation for DAFITI
    if (newSummaries.DAFITI.totalPedidos > 0) {
      newSummaries.DAFITI.percentualEntregas = 
        (newSummaries.DAFITI.entregues / newSummaries.DAFITI.totalPedidos) * 100;
    }

    // Calculate totals for ALL (excluding DAFITI)
  ...
  const RegionSummaryCard = ({ region, summary }: { region: string, summary: RegionSummary }) => {
    ...
    // Only show ALL when 'all' is selected, or show specific region when that region is selected
              ...
              <thead className="bg-[#ed5c0e] text-black">
                <tr>
                  {[
                    ...
                    <th
                      key={key}
                      className={`px-4 py-3 text-center text-base font-bold uppercase tracking-wider cursor-pointer ${
  index < array.length - 1 ? 'border-r border-gray-800' : ''
                  ...
                  <tr key={item.id} className="font-bold">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-center border-r border-gray-800 ">
                      {item.isEditing ? (
                        <input
                          type="text"
                          className="px-4 py-3 whitespace-nowrap text-center border-r border-gray-800 "
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-center border-r border-gray-800 ">{item.pendente}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-center border-r border-gray-800 ">
                      {item.isEditing ? (
                        <input
                          type="text"
